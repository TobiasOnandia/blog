...
export const FormPost = () => {
  ...
  const [state, formAction, isPending] = useActionState(
    async (_: void | null, formData: FormData) => {
      const { title, content } = window.Object.fromEntries(formData.entries());
  ...
  return (
    <form action={formAction} className="space-y-8 flex flex-col gap-4">
      {/* Sección básica */}
      <label className="flex flex-col gap-4 text-sm uppercase tracking-widest">
        Título principal
        <input
          type="text"
          name="title"
      ...
      <label className="  text-sm uppercase tracking-widest">
        Sección:
        <select className="bg-transparent ml-2 border border-black/20 px-4 py-2">
